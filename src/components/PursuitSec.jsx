    ...
    return (
        <section className="px-5 sm:px-10 md:px-[60px] pt-[90px] md:pt-[120px] lg:pt-[140px] relative overflow-x-clip">
            <div className="container max-w-[1140px] mx-auto relative z-20">
                ...
                <div className="grid grid-cols-3 gap-6 gap-y-9 md:mt-20 max-lg:hidden">
                    {pursuitSecList.map((item, index) => (
                        <div className="h-full col-span-1" key={index}>
                            <div className="flex flex-col justify-between h-full py-6 pl-6 lg:h-[270px]  pr-5 duration-300 border group rounded-2xl border-lightGreyPrimary shadow-[0_4px_16px_0_#0000000F] hover:shadow-none bg-lightBrownSecondary hover:bg-darkYellowPrimary sm:p-5 lg:pt-6 lg:px-6 lg:pb-7 lg:pr-7 hover:pb-6 hover:border-darkYellowSecondary">
                                <div className="flex flex-col gap-3">
            ...
            <img className="absolute bottom-0 -right-11 lg:right-0 z-0 md:translate-y-[86px] lg:translate-y-10 max-md:hidden md:h-[712px] lg:h-[846px]" src={temple} alt="temple" />
            <img className="absolute -bottom-12 lg:bottom-0 -left-20 lg:left-0 z-0 translate-y-[73px] max-md:hidden md:h-[358px] lg:h-[424px]" src={vector} alt="vector" />
        </section>
    )
}
